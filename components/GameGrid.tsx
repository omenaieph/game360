...
const GameCard: React.FC<GameCardProps> = ({ title, category, players, icon, previewUrl, onClick }) => {
  ...
  const handleMouseLeave = () => {
    if (videoRef.current) {
      ...
      // Optional: reset to start for next hover
      videoRef.current.currentTime = 0;
    }
  };
  ...
  return (
    <div 
      ...
      className="group relative bg-glass backdrop-blur-md rounded-2xl p-6 border border-glassBorder overflow-hidden transition-all duration-500 hover:scale-[1.03] hover:border-neonCyan/50 hover:shadow-[0_0_30px_rgba(0,229,255,0.15)] cursor-pointer"
    >
      ...
      <div className="relative z-10 flex flex-col items-center text-center transform transition-transform duration-500 group-hover:-translate-y-1">
        ...
        {/* Dynamic Footer: Players count swaps with Play Button */}
...
const GameModal: React.FC<{ game: Game | null; onClose: () => void }> = ({ game, onClose }) => {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 transition-opacity duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      ...
      {/* Modal Content */}
      <div className={`relative w-full max-w-2xl bg-void border border-glassBorder rounded-2xl shadow-[0_0_50px_rgba(0,229,255,0.15)] overflow-hidden transform transition-all duration-300 ${isVisible ? 'scale-100 translate-y-0' : 'scale-95 translate-y-8'}`}>
...
export const GameGrid: React.FC = () => {
  ...
  const games: Game[] = [
    ...
    { 
      ...
      // Matrix/Cyber code style falling blocks
